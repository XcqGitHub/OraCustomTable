...
@property (nonatomic, strong) NSMutableArray *headPointYArr;
@end
...
- (void)configUI{
    ...
    self.leftTableView.clipsToBounds = NO;
    self.leftTableView.layer.masksToBounds = NO;
    
}

- (void)layoutSubviews{
    [super layoutSubviews];
    _leftHeadView.frame = CGRectMake(0, 0,self.dataModel.firstColumnWidth, self.dataModel.firstRowHeight+40);
    ...
    _rightHeadView.frame = CGRectMake(0, 0,rightViewWidth, self.dataModel.firstRowHeight+40);
...
- (void)observerOffsetY:(CGFloat)offsetY {
    if (_headPointYArr.count == 0) {
        return;
    }
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        int index = 0;
        for (NSNumber *num in _headPointYArr) {
            if (offsetY >= num.floatValue) {
//                NSLog(@"index:%@  hidden:%@ ",@(index),@(true));
                [[NSNotificationCenter defaultCenter]postNotificationName:NSStringFromClass(self.class) object:@{@"index":@(index),@"hidden":@(true)}];
            } else {
//                NSLog(@"index:%@  hidden:%@ ",@(index),@(true));
                [[NSNotificationCenter defaultCenter]postNotificationName:NSStringFromClass(self.class) object:@{@"index":@(index),@"hidden":@(false)}];
            }
            index ++;
        }
    });
...
{
    return self.dataModel.firstRowHeight+40;
}
...
- (void)setDataModel:(ZQDataGridComponentModel *)dataModel {
    if (!dataModel) {
        return;
    }
    _dataModel = dataModel;
    if (_dataModel.headSectionNum.count == 0) {
        return;
    } else {
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0), ^{
            ...
            int lastNum = 0;
            CGFloat height = 0;
            for (NSNumber *num in _dataModel.headSectionNum) {
                for (int i = lastNum; i <= num.intValue; i++) {
                    height += ((NSNumber *)(self->_dataModel.rowHeightArr[i])).floatValue;
                }
                [arr addObject:@(height)];
                lastNum = num.intValue + 1;
            }
            _headPointYArr = arr;
        });
    }
